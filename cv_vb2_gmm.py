...
forder = np.arange(len(old_ind))[::-1]
X = X[:,forder]
...
"""VB Hyperparameter grids"""
sc_alphas = [1e-9,1e-7,1e-5]#,1e-2,1e-1,1
sc_gam_rates = [1e-5,1e-3,1e-1]
sc_gam_shapes = [1e-5,1e-3,1e-1]
sc_m0s = [1e-1,1,1e+1]
...
the_metric = []
highest_bound_ever = -np.infty
parameter_space = np.ndarray((len(sc_alphas),len(sc_m0s),len(sc_gam_rates),\
                            len(sc_gam_shapes),X.shape[-1]))
j = 1
for jj in np.arange(1,X.shape[-1]+1):
    
    Xrfe=X[:,:jj]
    #Xtrfe=Xt[:,:jj]
    n, d = Xrfe.shape
    highest_bound = -np.infty
    lb = []
    
    for kk,sc_alpha in enumerate(sc_alphas):
        for ll,sc_m0 in enumerate(sc_m0s):
            for mm,sc_gam_rate in enumerate(sc_gam_rates):
                for nn,sc_gam_shape in enumerate(sc_gam_shapes):
                    
                    "Define VB Model using Hyperparameters"
                    ...
                    bound = Q.compute_lowerbound()/jj
                    lb.append(bound)
                    parameter_space[kk,ll,mm,nn,jj-1] = bound
                    if lb[-1] > highest_bound:
                        highest_bound = lb[-1]
                        
                    if highest_bound > highest_bound_ever:
                        highest_bound_ever = highest_bound
                        best_vb = Q
                        best_hps = [d,sc_alpha,sc_m0,sc_gam_rate,sc_gam_shape]
                    print(j,'/',np.prod(parameter_space.shape),' - ',bound)    
                    j+=1
     #print()               
